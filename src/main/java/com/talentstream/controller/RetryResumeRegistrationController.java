...
public class RetryResumeRegistrationController {
	 ...
	 private static final Logger LOGGER=LoggerFactory.getLogger(RetryResumeRegistrationController.class);
	...
	public ResponseEntity<String> retryResumeRegistration() {
	    try {
	        ...
	        LOGGER.info("checking for size {}",failedApplicants.size());
	        for (Applicant applicant : failedApplicants) {
	            ...
	            try {
                    ...
                    LOGGER.info("REsume ID {}",userId);
	                // Update the applicant with the resume ID
	                applicant.setResumeId(userId);
	                applicantRepository.save(applicant);
	                
	                LOGGER.info("resume id  {}  saved in TS db",userId);
	               
	            } catch (Exception e) {
	                
	                LOGGER.info("Error registering applicant{}: {} " ,applicant.getEmail() ,e.getMessage());
	            }
	        }
