...
import com.talentstream.config.AESConfig;
...
public class JobRecruiterController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JobRecruiterController.class);
    ...
    private static final String NO_RECRUITER_FOUND="No recruiter found with ID: {}";
    ...
    public ResponseEntity<Object> login(@RequestBody RecruiterLogin loginRequest) throws Exception {
    	...
    	logger.info("decryptedBytes {} ",decryptedPassword);
    ...
    private ResponseEntity<Object> createAuthenticationToken(RecruiterLogin login, JobRecruiter recruiter)
        ...
        final String refreshToken=jwtTokenUtil.generateToken(userDetails.getUsername());
        return ResponseHandler.generateResponse2("Login successfully" + userDetails.getAuthorities(), HttpStatus.OK,
                new AuthenticationResponse(jwt,refreshToken), recruiter.getEmail(), recruiter.getCompanyname(),
    ...
    public ResponseEntity<Integer> getUnreadAlertCount(@PathVariable long recruiterId) {
        try {
            ...
            if (recruiter != null) {
            	...
            	//#change
                logger.warn(NO_RECRUITER_FOUND, recruiterId);
