...
import com.talentstream.dto.ApplicantSendOtpDTO;
...
public class RegisterController {
	...
	private static final String VALIDATION_ERROR="Validation errors occurred during registering new applicant: {}";
	@Autowired
	...
	@Autowired
	JobRecruiterController controller;
	@Autowired
	...
	public ResponseEntity<String> register(@Valid @RequestBody RegistrationDTO registrationDTO,
		...
		if (bindingResult.hasErrors()) {
			...
			logger.warn(VALIDATION_ERROR, responseBody);
	...
	public ResponseEntity<Object> login(@Valid @RequestBody LoginDTO loginDTO, BindingResult bindingResult)
	    ...
	    try {
	        ...
	        if (regsiterService.isGoogleSignIn(loginDTO)) {
	        	...
	        	//logger.warn("V4 is:{} ",loginDTO.getIv());
		    	//String decryptedPassword = controller.decrypt(loginDTO.getPassword(), loginDTO.getIv()).trim();
		        
		  //  	logger.info("Decryted password is: {} ",decryptedPassword);
	...
	private ResponseEntity<Object> createAuthenticationToken(LoginDTO loginDTO, Applicant applicant) throws Exception {
		try {
			if (regsiterService.isGoogleSignIn(loginDTO)) {
				...
				logger.info("Now I am at token gen");
	...
	public ResponseEntity<Object> refreshToken(@RequestBody RefreshTokenDTO refreshTokenDTO) {
	    try {
	        ...
	        if (isValid) {
	            ...
	            logger.info("returning new jwty token");
	        	return ResponseEntity.ok(response);
	            
	        } else {
	        	logger.info("returning token expired message");
	    	...
	    	logger.error("Invalid token in catch block");
	        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Invalid or expired token: " + e.getMessage());
	    } catch (Exception e) {
	       logger.error("returning error message while refreshing token");
	...
	public ResponseEntity<String> sendOtp(@Valid @RequestBody ApplicantSendOtpDTO applicantSendOtpDTO, BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			// Handle validation errors
			Map<String, String> fieldErrors = new LinkedHashMap<>();
		...
		String userEmail = applicantSendOtpDTO.getEmail();
		String userMobile = applicantSendOtpDTO.getMobilenumber();
	...
	public ResponseEntity<String> verifyOtp(@Valid @RequestBody OtpVerificationRequest verificationRequest,
		...
		try {
			...
			logger.info("Otp + Emial {} : {}",otp , email);
	...
	public ResponseEntity<String> setNewPassword(@Valid @RequestBody NewPasswordRequest request,
		...
		try {
			...
			logger.error("Error resetting password for email: {}", email);
