...
public class CompanyProfileController {
	...
	private static final String INTERNAL_MESG = "Internal server error occurred.";
	...
	public ResponseEntity<String> createCompanyProfile(@Valid @RequestBody CompanyProfileDTO companyProfileDTO,
			BindingResult bindingResult, @PathVariable Long jobRecruiterId) {
		...
		try {
			...
			logger.error("Internal server error occurred while saving company profile for job recruiter ID: {}",
					jobRecruiterId, e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(INTERNAL_MESG);
		}
	}

	@PutMapping("/{jobRecruiterId}/update-approval-status")
	public ResponseEntity<String> updateApprovalStatus(@PathVariable Long jobRecruiterId,
		...
		try {
			...
			logger.error("Internal server error occurred while updating approval status for job recruiter ID: {}",
					jobRecruiterId, e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(INTERNAL_MESG);
		}
	}
	...
	public ResponseEntity<CompanyProfileDTO> getCompanyProfileByRecruiterId(@PathVariable Long jobRecruiterId) {
		try {
			Optional<CompanyProfileDTO> companyProfileDTO = companyProfileService
					.getCompanyProfileDetailsByRecruiterId(jobRecruiterId);
			return companyProfileDTO.map(profile -> {
			...
			logger.info("Internal server error occurred while retrieving company profile for RecruiterID: {}",
					jobRecruiterId, e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
		}
	}
	...
	public ResponseEntity<String> updateCompanyProfile(@PathVariable Long jobRecruiterId,
			@Valid @RequestBody CompanyProfileDTO companyProfileDTO, BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			// Handle validation errors
			Map<String, String> fieldErrors = new LinkedHashMap<>();
			bindingResult.getFieldErrors().forEach(fieldError -> {
		...
		try {
			...
			logger.error("Error occurred while updating company profile details for job recruiter ID: {}",
					jobRecruiterId, ce);
			return ResponseEntity.status(ce.getStatus()).body(ce.getMessage());
		} catch (Exception e) {
			logger.error("Internal server error occurred while updating company profile for job recruiter ID: {}",
					jobRecruiterId, e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(INTERNAL_MESG);
		}
	}
	...
	public ResponseEntity<String> checkApprovalStatus(@PathVariable Long jobRecruiterId) {
		try {
			...
			logger.error("Internal server error occurred while checking approval status for job recruiter ID: {}",
					jobRecruiterId, e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(INTERNAL_MESG);
		}
	}
