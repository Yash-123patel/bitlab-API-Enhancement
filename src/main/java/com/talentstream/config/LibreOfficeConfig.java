...
public class LibreOfficeConfig {
    ...
    @Bean(destroyMethod = "stop") // Stops LibreOffice on shutdown
    public LocalOfficeManager officeManager() {
        if (officeManager == null) {  // Ensure only one instance
            officeManager = LocalOfficeManager.builder()
                    .install()
                    .portNumbers(8100)  // Ensure only one process runs on port 8100
                    .build();
            try {
                officeManager.start();
