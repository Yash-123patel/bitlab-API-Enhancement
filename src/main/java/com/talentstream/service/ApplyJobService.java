...
public class ApplyJobService {
	...
	private static final String COMP_MESG="Company information not found for the given ApplyJob";
	
	private static final String MESG="contains";
	
	private static final Logger LOGGER=LoggerFactory.getLogger(ApplyJobService.class);
...
//	
	...
	private void sendAlerts(ApplyJob applyJob, String applicantStatus, String cN, String jobTitle) {
		...
		LocalDateTime updatedChangeDateTime = currentChangeDateTime.plusHours(5).plusMinutes(30);
	...
	private void sendEmailToApplicant(String toEmail, String cN, String applicantStatus, String jobTitle) {
		...
		try {
			...
			String content = "Dear Applicant,\n\n" + "Thank you for applying for the position of " + jobTitle + " at "
					+ cN
	...
	private void saveStatusHistory(ApplyJob applyJob, String applicationStatus) {
		...
		LOGGER.info("before addition {} " , currentDateTime);
		
		LocalDateTime updatedDateTime = currentDateTime.plus(Duration.ofHours(5).plusMinutes(30));
		LOGGER.info("after addition  {}" ,updatedDateTime);
	...
	public List<AppliedApplicantInfoDTO> getAppliedApplicants2(long jobRecruiterId, MatchTypes matchTypes, String name,
		...
		LOGGER.info(matchTypes.getName());
		
		LOGGER.info(matchTypes.getMobilenumber());
		...
		try {
			filteredList = all.stream().filter(applicant -> (name == null
					|| applyMatchType(applicant.getName(), name, matchTypes.getName(), "is"))
					&& (email == null || applyMatchType(applicant.getEmail(), email, matchTypes.getEmail(), MESG))
					&& (mobileNumber == null || applyMobileType(applicant.getMobilenumber(), mobileNumber,
							matchTypes.getMobilenumber(), "is"))
					&& (jobTitle == null
							|| applyMatchType(applicant.getJobTitle(), jobTitle, matchTypes.getJobTitle(), MESG))
					&& (applicantStatus == null || applyMatchType(applicant.getApplicantStatus(), applicantStatus,
							matchTypes.getApplicantStatus(), MESG))
					&&

					(minimumQualification == null || applyMatchType(applicant.getMinimumQualification(),
							minimumQualification, matchTypes.getMinimumQualification(), MESG))
					&& (location == null
							|| applyMatchType(applicant.getLocation(), location, matchTypes.getLocation(), MESG))
					&& (minimumExperience == null || applyExperienceMatchType(applicant.getExperience(),
							minimumExperience, matchTypes.getMinimumExperience(), "lessThan"))).toList();
		} catch (Exception e) {
	...
	private boolean applyMatchType(String value, String filterValue, String matchValue, String matchType) {
		...
		if (matchValue.equalsIgnoreCase(MESG)) {
	...
	public Map<String, List<AppliedApplicantInfoDTO>> getAppliedApplicants(long jobRecruiterId) {
		...
		for (AppliedApplicantInfo appliedApplicantInfo : appliedApplicants) {
			...
			LOGGER.info("applicantKey {} " , applicantKey);
			...
			if (!applicantMap.containsKey(applicantKey)) {
				...
				try {
					...
					if (responseBody == null) {
					...
					} else if (responseBody instanceof JobDTO) {
						...
						LOGGER.info("Unexpected response body type: {}" , responseBody.getClass().getName());
					}
	...
	private AppliedApplicantInfoDTO mapToDTO(AppliedApplicantInfo appliedApplicantInfo) {
		...
		if (job != null) {
			...
			if (jobSkills != null) {
				List<String> skills = jobSkills.stream().filter(skill -> skill.getSkillName() != null) // Avoid null
																										// keys
						.map(RecuriterSkills::getSkillName).toList();
	...
	public List<ApplicantJobInterviewDTO> getApplicantJobInterviewInfoForRecruiterAndStatus(long recruiterId,
	...
	public List<Alerts> getAlerts(long applicantId) {
		...
		return alerts.stream().filter(alert -> !"New".equals(alert.getApplyJob().getApplicantStatus()))
				.toList();
	}
