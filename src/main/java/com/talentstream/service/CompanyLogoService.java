...
public class CompanyLogoService {
	...
	public String saveCompanyLogo(long jobRecruiterId, MultipartFile imageFile) throws IOException {
		...
		if (recruiter == null) {
			...
			try {
				...
				s3Client.putObject(new PutObjectRequest(bucketName, objectKey, imageFile.getInputStream(),
	...
	public byte[] getCompanyLogo(long jobRecruiterId) {
		try {
			...
			return IOUtils.toByteArray(inputStream); // Convert input stream to byte array
